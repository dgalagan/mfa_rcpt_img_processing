...
def obtain_input_points(img, quad_size=10):
    ...
    # Access MyImage object to get array
    ...
    around_img_cntr = [width_cntr, height_cntr + 10]
    ...
    if start_row < 0 or end_row > height:
        ...
        input_points = np.array([img_cntr, around_img_cntr])
...
def non_maximum_suppression(boxes, probs=None, overlapThres=0.3):
    ...
    while len(idxs) > 0:
        ...
        # box and the smalles (x, y) coordinates for the end of the bounding
        # box
        ...
        idxs = np.delete(idxs, np.concatenate(([last], np.where(overlap > overlapThres)[0])))
        
    # return only the bounding boxes that were picked
    return boxes[pick].astype("int")

def bb_intersection_over_union(boxA, boxB):
    # determine the (x, y)-coordinates of the intersaction rectangle
